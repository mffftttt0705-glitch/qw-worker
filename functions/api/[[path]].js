...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === '/api/products' && method === 'GET') {
      return await handleGetProducts(env, url);
    }
    ...
    if (path === '/api/handlers' && method === 'GET') {
      return await handleGetHandlers(env);
    }
    ...
    if (path === '/api/tip' && method === 'POST') {
      return await handleSendTip(env, authHeader, body);
    }
    ...
    if (path.startsWith('/api/orders/')) {
      ...
      if (orderId.endsWith('/dispatcher-confirm') && method === 'PUT') {
        const id = orderId.replace('/dispatcher-confirm', '');
        return await handleDispatcherConfirmComplete(env, authHeader, id);
      }
    ...
    if (userId) {
      ...
      if (user && user.role === 'admin') {
        ...
        if (path === '/api/admin/categories' && method === 'POST') {
          return await handleAdminCreateCategory(env, body);
        }
